...
/*Cuenta todos los bits a 1 del unsigned char
 *recorre todo el char con el for y va moviendo todos los bits a la izquierda
 *sumando si el ultimo bit es 1
 *
 */
...
int ipv4_route_lookup(ipv4_route_t *route, ipv4_addr_t addr) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        //si la mascara no es igual a la de la subred devolvemos 1
        if ((addr[i] & route->subnet_mask[i]) != route->subnet_addr[i]) {
            return -1;
        }
        //contabilizamos los bits a 1 con switch lookup
